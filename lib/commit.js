...
Commit.prototype.amendWithSignature = function (
  ...
  let createCommitPromise = Promise.all(promises)
    ...
    .then(function (commitTreeResult) {
      ...
      const truthyArgs = {};
      if (author) {
        truthyArgs.author = author;
      }
      if (committer) {
        truthyArgs.committer = committer;
      }
      if (messageEncoding) {
        truthyArgs.messageEncoding = messageEncoding;
      }
      if (message) {
        truthyArgs.message = message;
      }
      if (treeObject) {
        truthyArgs.tree = treeObject;
      }
